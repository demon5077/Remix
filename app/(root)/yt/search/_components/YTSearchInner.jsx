...
export default function YTSearchInner() {
  ...
  return (
    <div className="px-5 md:px-8 py-8 min-w-0">
      {/* Search input */}
      <form onSubmit={handleSubmit} className="relative max-w-xl mb-8">
        <input value={query} onChange={handleChange} placeholder="Search YouTube — songs, artists, videos…"
          autoFocus
          className="w-full h-12 pr-14 pl-4 rounded-xl text-sm outline-none transition-all duration-200"
          style={{
            background: "rgba(18,18,32,0.9)", border: "1px solid rgba(255,0,60,0.15)",
        ...
        <div className="mb-6 p-4 rounded-xl flex gap-3 items-start"
          ...
          <div>
            ...
            <p className="text-xs mt-1 leading-relaxed" style={{ color: "#8888aa" }}>
              Add <code className="px-1.5 py-0.5 rounded text-[10px]"
                style={{ background: "rgba(255,255,255,0.06)", color: "#ccccee" }}>
                NEXT_PUBLIC_RAPIDAPI_KEY=your_key
              </code> to <code className="px-1 rounded text-[10px]"
                style={{ background: "rgba(255,255,255,0.06)", color: "#ccccee" }}>.env.local</code>
              {" "}— free key at{" "}
              <a href="https://rapidapi.com/ytjar/api/yt-api" target="_blank" rel="noopener noreferrer"
                className="underline" style={{ color: "#FF003C" }}>rapidapi.com</a>
            </p>
          </div>
        </div>
      )}
      ...
      {results.length > 0 && (
        <p className="remix-section-title mb-5">{results.length} results for "{query}"</p>
      )}

      {results.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {results.map(item => <YTCard key={item.id} item={item} layout="grid" />)}
        </div>
      )}
